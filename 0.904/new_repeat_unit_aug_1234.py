"""
new_repeat_unit_aug_1234.py — TETRAMER extension of the LB 0.904 winner.

Same 4-phase pipeline as new_repeat_unit_aug.py (LB 0.904), EXCEPT Chemprop
training uses graphs at FOUR chain lengths per polymer instead of three:
[monomer, dimer, trimer, TETRAMER]. Each length is a structurally different
graph → additional training signal for D-MPNN repetition-invariance.

Peer-reviewed: "Learning Repetition-Invariant Representations for Polymer
Informatics" (arXiv 2505.10726, 2025). Chain-length augmentation gives
coherent embeddings and improves polymer property predictions, with gains
reported up to n=60.

Baseline: LB 0.904 with CHAIN_LENGTHS_AUG=[1,2,3] (new_repeat_unit_aug.py).
Target:   LB > 0.904, expected 0.905-0.906 (top-5).
Risk:     Tetramer graphs are ~33% larger, batch-norm heterogeneity may add
          noise. Expected EV +0.0005 LB, ~40% chance of +0.001+, ~25% chance
          flat, ~25% chance of -0.001 regression.
...
2. **Chemprop 3-seed multitask, TETRAMER-AUGMENTED [1,2,3,4]** — 5-fold ×
   3-seed bagged D-MPNN. Training: each polymer contributes graphs at chain
   lengths [1, 2, 3, 4] (monomer, dimer, trimer, tetramer) as separate
   datapoints with same target. Val: monomer only. [1,2,3] variant hit
   LB 0.904; this extends to 4 chain lengths.
...
============================================================================
  ...
  work_0904_repeat_unit_aug_1234/  — all intermediates + logs (isolated
                                     dir; will NOT clash with 0.904 cache)
  ...
  submission.csv                 — FINAL submission (tetramer) — copy of Phase 4
                                   (on Kaggle: /kaggle/working/submission.csv)
...
- Phase 1 (LGB, ~15-20 min): skipped if lgb_maxwell/submission.csv exists.
- Phase 2 (Chemprop, ~8-9h on T4): resumes per fold via checkpoint_fold_k.pkl.gz;
  skipped entirely if refit_test_preds.pkl.gz already exists.
- Phases 3-4: cheap, always rerun.

Delete work_0904_repeat_unit_aug_1234/<phase>/ to force re-run of that phase.

============================================================================
WALL TIME (fresh run, no cache) — TETRAMER SCALES 1.67× vs [1,2,3]
============================================================================

Reference: [1,2,3] variant ran 5.24h on Kaggle T4 (LB 0.904).
Tetramer adds ~67% more training data (6N atoms → 10N atoms per polymer per
epoch) plus larger per-graph forward-pass cost.

  Phase 1 (LGB features + train):  ~15-20 min
  Phase 2 (Chemprop 3-seed):       ~8-10 h   (T4 GPU)
                                    ~15-20 h (CPU — DO NOT run on CPU)
  Phase 3 (blend NNLS):             <1 sec
  Phase 4 (Koopmans post-fit):      <5 sec

  Total: ~9-10 h on Kaggle T4. Fits 12h notebook limit with ~2-3h buffer.
  Requires GPU accelerator — set in Kaggle sidebar before running.
...
# NOTE: __file__ does NOT exist inside a Jupyter/Kaggle notebook kernel, so the
# original `Path(__file__)` lines raise NameError before anything else runs.
# This block works in a notebook, in `!python reproduce.py`, and locally.

ON_KAGGLE = Path("/kaggle/input").exists()

try:
    _HERE = Path(__file__).resolve().parent
except NameError:                      # notebook kernel — no __file__
    _HERE = Path.cwd()


def _find_data_dir() -> Path:
    """Locate the directory that actually contains train.csv + test.csv."""
    candidates = []
    if ON_KAGGLE:
        # competition data is normally at /kaggle/input/<comp-slug>/,
        # but some hosts nest it one level deeper.
    ...
    raise FileNotFoundError(
        "Could not find a directory containing train.csv and test.csv. "
        "Set DATA_DIR manually. On Kaggle, run `!ls -R /kaggle/input | head -50` "
        "to see the actual mount path."
    )
...
# /kaggle/input is READ-ONLY; /kaggle/working is the only writable persistent dir.
WORK_DIR = (Path("/kaggle/working") if ON_KAGGLE else _HERE) / "work_0904_repeat_unit_aug_1234"

# Kaggle expects the graded file at exactly /kaggle/working/submission.csv
FINAL_SUB_PATH = Path("/kaggle/working/submission.csv") if ON_KAGGLE else _HERE / "submission_repeat_unit_aug_1234.csv"
...
# ---- Chemprop (Phase 2) — REPEAT-UNIT augmentation, EXTENDED TO TETRAMER ----
# Each training polymer contributes graphs at multiple chain lengths.
# Peer-reviewed for polymer property prediction (arXiv 2505.10726).
# Each length is a STRUCTURALLY DIFFERENT graph → strong learning signal,
# unlike random-SMILES aug which produces the same graph.
# Val + test predict on monomer only (canonical) to match test-time flow.
# CHANGED FROM [1,2,3] (LB 0.904 winner) TO [1,2,3,4] (add tetramer).
CHAIN_LENGTHS_AUG = [1, 2, 3, 4]    # monomer + dimer + trimer + TETRAMER

MODEL_SEEDS = (42, 43, 44)          # 3-seed bag (unchanged from 0.904 winner)
D_H = 300
DEPTH = 4
MP_DROPOUT = 0.05
FFN_HIDDEN = 300
...
WARMUP_EPOCHS = 2
NUM_WORKERS = 0
DEVICE = "gpu" if torch.cuda.is_available() else "cpu"   # auto: set accelerator in Kaggle sidebar
...
def polymer_to_multimer(smi: str, n_units: int) -> str:
    """Extend *A* → n-mer chain head-to-tail. Returns canonical SMILES.
    n_units=1 returns input (monomer). n_units=2 returns dimer, etc.
    Falls back to input SMILES on any structural issue."""
    if n_units <= 1:
        return smi
    m = Chem.MolFromSmiles(smi)
    if m is None:
...
def _chain_length_variants(canon_smi: str, chain_lengths: list) -> list:
    """Return list of RDKit Mol objects, one per chain length.
    Deduplicates if two lengths produce the same SMILES (rare, but happens for
    already-linear polymers where dimer canonicalizes to monomer)."""
    variants = []
    seen = set()
    for n in chain_lengths:
        multimer_smi = polymer_to_multimer(canon_smi, n)
        if multimer_smi in seen:
            continue
        seen.add(multimer_smi)
        mol = Chem.MolFromSmiles(multimer_smi)
        if mol is not None:
            variants.append(mol)
    return variants
...
def make_train_val_datasets_chemprop(canons, y, train_idxs, val_idxs, featurizer):
    """Train set: each polymer contributes graphs at CHAIN_LENGTHS_AUG lengths
    (monomer + dimer + trimer). Val set: monomer only (test-time flow)."""
    train_pts = []
    n_variants_total = 0
    for i in train_idxs:
        variants = _chain_length_variants(canons[i], CHAIN_LENGTHS_AUG)
        for mol in variants:
            train_pts.append(data.MoleculeDatapoint(mol=mol, y=y[i]))
    ...
    for i in val_idxs:
        ...
        val_pts.append(data.MoleculeDatapoint(mol=m, y=y[i]))
...
def make_full_dataset_chemprop(canons, y, featurizer):
    """Refit dataset: each polymer contributes graphs at CHAIN_LENGTHS_AUG lengths
    (matches CV train-set construction)."""
    all_pts = []
    for i, smi in enumerate(canons):
        variants = _chain_length_variants(smi, CHAIN_LENGTHS_AUG)
        for mol in variants:
            all_pts.append(data.MoleculeDatapoint(mol=mol, y=y[i]))
...
def phase2_chemprop_3seed(tr: pd.DataFrame, te: pd.DataFrame, log: logging.Logger) -> Path:
    ...
    log.info("=" * 60)
    log.info(f"PHASE 2: Chemprop 3-seed multitask — REPEAT-UNIT AUG lengths={CHAIN_LENGTHS_AUG}")
    log.info("=" * 60)
...
# ============================================================================
# PHASE 4: KOOPMANS BANDGAP POST-FIT  — expected FINAL LB 0.905-0.906
# ============================================================================
...
def phase4_koopmans_postfit(blend_sub_dir: Path, chemprop_dir: Path,
    ...
    log.info("=" * 60)
    log.info("PHASE 4: Koopmans bandgap post-fit (Egc ≈ Ei - Eea)  — expected FINAL LB 0.905-0.906")
    log.info("=" * 60)
...
def main():
    ...
    log.info("=" * 60)
    log.info(" TETRAMER PIPELINE — extending LB 0.904 with CHAIN_LENGTHS_AUG=[1,2,3,4]")
    log.info("=" * 60)
    ...
    # ---- Copy final submission to the graded location ----
    final_sub_src = koopmans_dir / "submission.csv"
    shutil.copy2(final_sub_src, WORK_DIR / "submission.csv")
    shutil.copy2(final_sub_src, FINAL_SUB_PATH)
    final_sub_dst = FINAL_SUB_PATH

    # ---- Sanity-check against sample_submission.csv if the host provides one ----
    sample_path = DATA_DIR / "sample_submission.csv"
    check = pd.read_csv(final_sub_dst)
    n_nan = int(check["target"].isna().sum())
    log.info(f"submission rows={len(check)}  cols={list(check.columns)}  NaNs={n_nan}")
    if n_nan:
        log.warning(f"{n_nan} NaN predictions — Kaggle will reject these. Investigate before submitting.")
    if sample_path.exists():
        samp = pd.read_csv(sample_path)
        if list(samp.columns) != list(check.columns):
            log.warning(f"COLUMN MISMATCH: sample={list(samp.columns)} vs ours={list(check.columns)}")
        if len(samp) != len(check):
            log.warning(f"ROW COUNT MISMATCH: sample={len(samp)} vs ours={len(check)}")
        if set(samp["id"]) != set(check["id"]):
            log.warning("ID SET MISMATCH vs sample_submission.csv")
    else:
        log.info("no sample_submission.csv found — verify format manually before submitting")
    ...
    log.info(f"Expected LB: 0.905-0.906 (extending 0.904 winner with tetramer)")
    log.info("=" * 60)
