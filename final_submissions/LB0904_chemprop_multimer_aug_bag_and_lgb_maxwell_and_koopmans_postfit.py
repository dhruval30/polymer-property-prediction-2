"""
new_repeat_unit_aug.py — 0.902 pipeline with REPEAT-UNIT augmentation on Chemprop.

Same 4-phase pipeline as new.py, EXCEPT Chemprop training uses graphs at
multiple chain lengths per polymer (monomer + dimer + trimer). Each length
is structurally DIFFERENT graph → strong training signal for D-MPNN
...
Informatics" (arXiv 2505.10726, 2025). Reports that chain-length augmentation
gives coherent embeddings and improves polymer property predictions.

Target: LB > 0.902, ideally 0.904-0.906 (top-6).
...
2. **Chemprop 3-seed multitask, REPEAT-UNIT AUGMENTED** — 5-fold × 3-seed
   bagged D-MPNN. Training: each polymer contributes graphs at chain lengths
   [1, 2, 3] (monomer, dimer, trimer) as separate datapoints with same target.
   Val: monomer only. Baseline non-aug solo LB reference: 0.892.
...
WORK_DIR = (Path("/kaggle/working") if ON_KAGGLE else _HERE) / "work_0902_repeat_unit_aug"

# Kaggle expects the graded file at exactly /kaggle/working/submission.csv
FINAL_SUB_PATH = Path("/kaggle/working/submission.csv") if ON_KAGGLE else _HERE / "submission_repeat_unit_aug.csv"
...
# ---- Chemprop (Phase 2) — REPEAT-UNIT augmentation ----
...
CHAIN_LENGTHS_AUG = [1, 2, 3]       # monomer + dimer + trimer
