"""
new_chainext.py — 0.902 pipeline with LGB chain-ext (mono + trimer) instead of mono-only.

Same 4-phase pipeline as new.py (LGB → Chemprop → NNLS blend → Koopmans),
but LGB now uses mono + trimer features (matches exp_chain_ext_catboost stack).
Chain-ext LGB solo was 0.894 vs mono 0.860 (+0.034), so this should give a
stronger LGB base for the 2-way NNLS blend.
...
1. **LGB (mono + trimer + Maxwell)** — LightGBM per-target on mono + trimer
   fingerprints + RDKit descriptors + 14 aux matrix-completion features.
   Chain-extension: repeat each polymer *A* → *AAA* (trimer). Trimer
   fingerprints pick up inter-monomer junction patterns. Maxwell EPS↔Nc
   post-fit as physics prior. Solo LB reference: 0.894 (up from 0.860 mono).
...
WORK_DIR = (Path("/kaggle/working") if ON_KAGGLE else _HERE) / "work_0902_chainext"

# Kaggle expects the graded file at exactly /kaggle/working/submission.csv
FINAL_SUB_PATH = Path("/kaggle/working/submission.csv") if ON_KAGGLE else _HERE / "submission_chainext.csv"
...
# ---- LGB (Phase 1) — chain-ext (mono + trimer) ----
# Trimer: repeat *A* → *AAA* (3 units head-to-tail). Trimer fingerprints
# encode inter-monomer junctions monomer fingerprints can't see. Matches
# exp_chain_ext_catboost feature stack (mono 7 families + trimer 5 families).
CHAIN_N_UNITS = 3
MORGAN2_NBITS = 2048
MORGAN3_NBITS = 2048
ATOMPAIR_NBITS = 2048
...
def build_feature_bundle_lgb(canon_smiles: list[str], log: logging.Logger) -> dict:
    """Mono + trimer feature stack for LGB (matches exp_chain_ext_catboost).
      ...
      Trimer (5 families ~4982): desc + morgan-r2 + maccs + atompair + avalon
                                 (drops weak morgan-r3 + toptorsion from trimer)
      Grand total ~14060 features per polymer."""
    ...
    for name, fn in [
        ...
        ("atompair_c_mono",   lambda: np.stack([compute_atompair_count(s, ATOMPAIR_NBITS) for s in tqdm(smis_mono, desc="mono atom-pair", ncols=100)])),
        ("toptorsion_c_mono", lambda: np.stack([compute_toptorsion_count(s, TOPTORSION_NBITS) for s in tqdm(smis_mono, desc="mono top-torsion", ncols=100)])),
    ...
    for name, fn in [
        ...
        ("atompair_c_tri", lambda: np.stack([compute_atompair_count(s, ATOMPAIR_NBITS) for s in tqdm(smis_tri, desc="tri atom-pair", ncols=100)])),
...
def get_or_build_features_lgb(all_canon: list[str], cache_path: Path, log: logging.Logger) -> dict:
    key = hashlib.md5(
        (str(sorted(set(all_canon))) +
         f"m2={MORGAN2_NBITS};m3={MORGAN3_NBITS};ap={ATOMPAIR_NBITS};"
         f"tt={TOPTORSION_NBITS};av={AVALON_NBITS};chain={CHAIN_N_UNITS}"
         ).encode()
    ).hexdigest()[:12]
