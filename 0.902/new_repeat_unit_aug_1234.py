"""
new_repeat_unit_aug_1234.py — extended repeat-unit aug: [monomer, dimer, trimer, tetramer].

Extension of new_repeat_unit_aug.py (which hit LB 0.904 with chain lengths
[1, 2, 3]). Adds tetramer (n=4) to give the D-MPNN one more chain length
signal. More graph diversity per polymer → stronger repetition-invariance.

Target: LB > 0.904, ideally 0.905-0.907 (top-5).
...
2. **Chemprop 3-seed multitask, REPEAT-UNIT AUGMENTED [1,2,3,4]** — 5-fold ×
   ...
   datapoints with same target. Val: monomer only. Prev [1,2,3] hit LB 0.904.
...
WORK_DIR = (Path("/kaggle/working") if ON_KAGGLE else _HERE) / "work_0902_repeat_unit_aug_1234"
...
CHAIN_LENGTHS_AUG = [1, 2, 3, 4]    # monomer + dimer + trimer + tetramer (extending winning [1,2,3])
