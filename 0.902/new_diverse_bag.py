"""
new_diverse_bag.py — 0.902 pipeline with 5-config DIVERSE Chemprop bag.

Same 4-phase pipeline as new.py, EXCEPT Chemprop bag members use DIFFERENT
architectures (varied d_h, depth, dropout) instead of identical config with
different seeds. Real error decorrelation instead of noise averaging.

Target: LB > 0.902, ideally 0.905.
...
2. **Chemprop 5-config DIVERSE multitask** — 5-fold × 5-config bagged D-MPNN.
   Each bag member has DIFFERENT hyperparameters (d_h, depth, dropout) so
   errors decorrelate across bag members. Baseline 3-seed same-config: 0.892.
...
============================================================================
  ...
  work_0902/                     — all intermediates + logs
      ...
      chemprop_diverse_bag/      — Phase 2 outputs
...
WORK_DIR = (Path("/kaggle/working") if ON_KAGGLE else _HERE) / "work_0902_diverse_bag"

# Kaggle expects the graded file at exactly /kaggle/working/submission.csv
FINAL_SUB_PATH = Path("/kaggle/working/submission.csv") if ON_KAGGLE else _HERE / "submission_diverse_bag.csv"
...
# ---- Chemprop (Phase 2) — DIVERSE 5-config bag ----
# Instead of 3 seeds with identical config (noise averaging), use 5 configs
# with varied (d_h, depth, mp_dropout, ffn_hidden). Different capacity + depth
# + regularization → genuine error decorrelation across bag members.
# NOTE: FFN input_dim MUST match d_h (encoder output), so ffn_hidden == d_h.
CHEMPROP_BAG_CONFIGS = [
    dict(seed=42, d_h=300, depth=4, mp_dropout=0.05, ffn_hidden=300),  # baseline (was 0.902 pipeline)
    dict(seed=43, d_h=300, depth=3, mp_dropout=0.05, ffn_hidden=300),  # shallower
    dict(seed=44, d_h=400, depth=4, mp_dropout=0.10, ffn_hidden=400),  # wider + more reg
    dict(seed=45, d_h=300, depth=5, mp_dropout=0.05, ffn_hidden=300),  # deeper
    dict(seed=46, d_h=350, depth=4, mp_dropout=0.05, ffn_hidden=350),  # mid-width
]

# Fixed shared params (don't vary across bag members)
FFN_LAYERS = 2
FFN_DROPOUT = 0.05
BATCH_NORM = True
MAX_EPOCHS = 60
...
def build_chemprop_model(output_transform=None, d_h: int = 300, depth: int = 4,
                          mp_dropout: float = 0.05, ffn_hidden: int = 300) -> MPNN:
    """Build a Chemprop MPNN with per-config arch params (for diverse bag)."""
...
def train_chemprop_cv_model(train_dset, val_dset, scaler, cfg, ctx, log):
    """cfg is a dict from CHEMPROP_BAG_CONFIGS: {seed, d_h, depth, mp_dropout, ffn_hidden}."""
    ...
    model = build_chemprop_model(
        output_transform=output_transform,
        d_h=cfg["d_h"], depth=cfg["depth"],
        mp_dropout=cfg["mp_dropout"], ffn_hidden=cfg["ffn_hidden"],
    )
...
def train_chemprop_refit_model(full_dset, scaler, cfg, n_epochs, test_canon_unique,
                                featurizer, ctx, log):
    """cfg is a dict from CHEMPROP_BAG_CONFIGS."""
    L.seed_everything(cfg["seed"], workers=True)
...
def train_chemprop_fold_bag(fold_k, canons, y, train_idxs, val_idxs, featurizer, log):
    """Train the DIVERSE bag on one fold. Iterates over CHEMPROP_BAG_CONFIGS."""
    log.info("=" * 60)
    log.info(f"CHEMPROP FOLD {fold_k}  ({len(CHEMPROP_BAG_CONFIGS)}-config diverse bag)")
    log.info(f"n_train_canon={len(train_idxs)}   n_val_canon={len(val_idxs)}")

    val_preds_per_bag, best_epochs, wall_times = [], [], []
    for si, cfg in enumerate(CHEMPROP_BAG_CONFIGS):
        ctx = (f"CP fold {fold_k} cfg{si+1}/{len(CHEMPROP_BAG_CONFIGS)} "
               f"(seed={cfg['seed']} d_h={cfg['d_h']} depth={cfg['depth']} "
               f"drop={cfg['mp_dropout']})")
        ...
        val_preds, best_epoch, wall = train_chemprop_cv_model(train_dset, val_dset, scaler, cfg, ctx, log)
        val_preds_per_bag.append(val_preds)
        best_epochs.append(best_epoch)
        wall_times.append(wall)

    val_preds_avg = np.mean(np.stack(val_preds_per_bag, axis=0), axis=0)
    val_true = y[val_idxs]
    return {
        "fold_k": fold_k, "val_idxs": val_idxs,
        "val_preds_per_bag": val_preds_per_bag,
        "val_preds_avg": val_preds_avg,
        "val_true": val_true,
        "best_epochs_per_bag": best_epochs,
        "wall_times_per_bag_min": wall_times,
        "bag_configs_used": [dict(c) for c in CHEMPROP_BAG_CONFIGS],
    }


def phase2_chemprop_diverse_bag(tr: pd.DataFrame, te: pd.DataFrame, log: logging.Logger) -> Path:
    """Full Chemprop DIVERSE-bag pipeline with per-fold checkpointing."""
    phase_dir = WORK_DIR / "chemprop_diverse_bag"
    phase_dir.mkdir(parents=True, exist_ok=True)
    sub_path = phase_dir / "submission.csv"
    oof_path = phase_dir / "oof.csv"
    ...
    log.info("=" * 60)
    log.info(f"PHASE 2: Chemprop DIVERSE {len(CHEMPROP_BAG_CONFIGS)}-config bag multitask")
    log.info("=" * 60)
    for i, cfg in enumerate(CHEMPROP_BAG_CONFIGS):
        log.info(f"  bag[{i}] = {cfg}")
    ...
    # ---- 5-fold CV × diverse bag with per-fold checkpointing ----
    ...
    for k, (tri, vai) in enumerate(splits):
        ...
        if cp_path.exists():
            ...
            continue
        result = train_chemprop_fold_bag(k, canons, y, tri, vai, featurizer, log)
    ...
    log.info(f"PER-TARGET OOF R² (Chemprop diverse {len(CHEMPROP_BAG_CONFIGS)}-config bag)")
    ...
    # ---- Refit epochs (median across all bag×fold trainings) ----
    all_best_epochs = [e for r in fold_results for e in r["best_epochs_per_bag"]]
    ...
    if refit_path.exists():
        ...
        test_preds_per_bag = []
        for si, cfg in enumerate(CHEMPROP_BAG_CONFIGS):
            ctx = (f"CP REFIT cfg{si+1}/{len(CHEMPROP_BAG_CONFIGS)} "
                   f"(seed={cfg['seed']} d_h={cfg['d_h']} depth={cfg['depth']})")
            ...
            test_preds, wall = train_chemprop_refit_model(
                full_dset, scaler, cfg, refit_epochs, test_canon_unique, featurizer, ctx, log,
            )
            test_preds_per_bag.append(test_preds)
        test_preds_avg = np.mean(np.stack(test_preds_per_bag, axis=0), axis=0)
        with gzip.open(refit_path, "wb") as f:
            pickle.dump({"test_preds_avg": test_preds_avg,
                          "test_preds_per_bag": test_preds_per_bag}, f,
...
def main():
    ...
    chemprop_dir = phase2_chemprop_diverse_bag(tr, te, log)
