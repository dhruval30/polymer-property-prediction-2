"""
exp_chain_ext_catboost.py — CatBoost on chain-ext v1 feature stack + Maxwell prior.

============================================================================
WHY THIS EXISTS
============================================================================

Chain-ext LGB v1 hit LB 0.894 solo. This script tests whether the SAME
feature engineering (mono + trimer + aux + Maxwell) with a DIFFERENT tree
family (CatBoost oblivious trees instead of LightGBM leaf-wise) gives
error patterns that blend well with LGB.

Design:
  - Same fold split (seed=42) → OOFs are directly NNLS-blendable with
    chain-ext LGB v1 and other bases
  - Same feature stack (mono + trimer + aux)
  - Same Maxwell EPS↔Nc post-fit
  - CatBoost hparams from Round 1: iterations=4000, depth=8, lr=0.03,
    l2_leaf_reg=3.0, +early stopping 200. No tuning (v2 lesson: don't
    tune models with positive OOF-LB gap)

Wall time: ~1-2h on Mac CPU (CatBoost ~2-3× slower than LGB on this data
size + feature dim). Kaggle-notebook-compatible.

Expected LB solo: 0.870-0.890 (CatBoost on chain-ext should be similar to
LGB, maybe slightly lower). Blend potential: if errors are structurally
different from LGB, +0.001 to +0.005 over chain-ext LGB alone.

============================================================================
ORIGINAL v1 DESCRIPTION FOLLOWS (feature engineering)
============================================================================
...
the trimer, and concatenates as separate feature families in a CatBoost
pipeline. Everything else (GroupKFold, aux matrix-completion features,
Maxwell EPS↔Nc post-fit) is identical to `exp_maxwell_prior_catboost.py`.
...
EXP_NAME = "exp_chain_ext_catboost"
...
def train_one_target(
    ...
    for k, (tri, vai) in enumerate(fold_bar):
        model = cb.CatBoostRegressor(iterations=N_ESTIMATORS, **CAT_PARAMS)
        model.fit(
            X_tr[tri], y[tri],
        ...
        oof[vai] = pred_va
        best_it = int(model.get_best_iteration()) if model.get_best_iteration() else N_ESTIMATORS
        best_iters.append(best_it)
        r2 = r2_score(y[vai], pred_va)
        fold_r2s.append(float(r2))
        fold_bar.set_postfix(fold=k, best_iter=best_it, r2=f"{r2:.4f}")
        log.info(f"[{target}] fold {k}: best_iter={best_it:>4d}   "
                 f"R²={r2:.4f}   n_val={len(vai)}")

    oof_r2 = float(r2_score(y, oof))
    log.info(f"[{target}] OOF R² (CAT only, pre-Maxwell) = {oof_r2:.4f}   (fold mean {np.mean(fold_r2s):.4f})")
    ...
    full_booster = cb.CatBoostRegressor(iterations=refit_iters, **CAT_PARAMS)
    full_booster.fit(X_tr, y, verbose=False)
    test_pred = full_booster.predict(X_te)

    # Per-family gain breakdown to see if trimer features are earning their spot
    imp = full_booster.get_feature_importance(type="FeatureImportance")
...
def main():
    ...
    # Final comparison vs chain-ext v1 LGB reference (LB 0.894)
    log.info("=" * 60)
    log.info("FINAL PER-TARGET OOF R²  (CatBoost + chain-ext + Maxwell)  vs chain-ext v1 LGB reference")
    log.info("=" * 60)
    v1_ref = {"eea": 0.8734, "egb": 0.9087, "egc": 0.9023,
              "ei":  0.8041, "eps": 0.8218, "nc":  0.8471, "tg":  0.9063}
    log.info(f"  {'target':>6s}  {'CAT-chain':>10s}  {'LGB v1':>10s}  {'delta':>8s}  {'tri gain':>10s}")
    for t in TARGETS:
        r2 = per_target[t]["oof_r2"]
        ref = v1_ref[t]
        d = r2 - ref
        log.info(f"  {t:>6s}  {r2:>10.4f}  {ref:>10.4f}  {d:>+8.4f}  {per_target[t]['tri_gain_share']:>9.1f}%")
    v1_mean = float(np.mean(list(v1_ref.values())))
    log.info(f"  {'MEAN':>6s}  {mean_r2:>10.4f}  {v1_mean:>10.4f}  {mean_r2 - v1_mean:>+8.4f}")
    log.info(f"  (chain-ext v1 LGB LB reference: 0.894)")
