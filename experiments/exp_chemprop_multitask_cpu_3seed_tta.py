"""
exp_chemprop_multitask_cpu_3seed_tta.py — 3-seed Chemprop + SMILES-enumeration TTA at inference.

============================================================================
WHAT THIS IS
============================================================================

Extends `exp_chemprop_multitask_cpu_3seed.py` (LB 0.892 solo, contributes to
LB 0.897 blend) with SMILES-enumeration test-time augmentation.

For each val row (OOF) and each test row (submission), we generate N_TTA=10
randomized-atom-order variants of the same molecule, run inference on each,
and average the N_TTA predictions. Chemprop's D-MPNN is theoretically
permutation-invariant but empirically shows 0.005-0.015 R² variance across
atom orderings due to floating-point summation in message aggregation and
batch-norm statistics. Averaging pulls that variance out.

============================================================================
WHY THIS PRESERVES OUR +0.028 OOF-LB GAP
============================================================================

TTA doesn't touch training — the trained Chemprop models are identical to
the baseline 3-seed. Only inference changes.

Effect:
  - Chemprop's OOF: should be nearly identical to baseline (guard rail
    verifies this — see below)
  - Chemprop's LB: should improve slightly (avg over noisy inference views
    is smoother than any one view)
  - Blend NNLS weights: unchanged (recompute from OOFs — same result)
  - Bias-mitigation constants (CHEMPROP_WEIGHT_FLOOR=0.40, +0.15 bias):
    still apply, since we only shift LB not OOF

This is the exact opposite of the failure pattern that killed v2/v3/v3fixed:
those maximized OOF and destroyed the LB gap. TTA is post-training — no
optimization surface, no gap to destroy.

============================================================================
GUARD RAIL (see main() end)
============================================================================

After finishing, compare TTA-OOF vs baseline 3-seed OOF per target:
  - If any target's TTA-OOF drops by more than 0.005 → don't ship, model
    is genuinely SMILES-order-sensitive on that target (bug)
  - If TTA-OOF is within ±0.003 across all targets → expected, ship
  - If TTA-OOF improves by more than 0.005 on average → suspicious, verify

============================================================================
NOTE ON CHECKPOINTS
============================================================================

The baseline 3-seed script cached VAL PREDICTIONS (not model weights) in
its per-fold checkpoints. So this script must retrain from scratch — we
can't just re-run inference on saved models. Wall time is therefore
similar to the baseline plus TTA inference overhead.

Baseline: 5 folds × 3 seeds × 60 epochs + 3 refits × ~50 epochs = 225 min
Additional TTA inference: ~2h for CV + ~30 min for test = ~2.5h
**Total: ~5-6 hours on Mac CPU.**

Same 5-fold split_seed=42 and same 3 model seeds (42, 43, 44) as baseline
→ OOF is directly blend-alignable.

============================================================================
ORIGINAL 3seed HEADER FOLLOWS
============================================================================
...
EXP_NAME = "exp_chemprop_multitask_cpu_3seed_tta"
EXP_DIR = REPO / "results" / EXP_NAME

# SMILES-enumeration TTA — number of randomized atom orderings per molecule
# at inference time. Standard range: 5-10, diminishing returns past 10.
N_TTA = 10
...
# ============================================================================
# TTA HELPERS  (SMILES-enum via RDKit atom-order randomization)
# ============================================================================

def make_random_variant_mol(m: Chem.Mol, seed: int) -> Chem.Mol:
    """Return a Mol with randomly-shuffled atom indices. Graph is identical;
    Chemprop's featurizer produces different tensor representations for the
    same underlying molecule, which is what TTA exploits."""
    n_atoms = m.GetNumAtoms()
    rng = np.random.default_rng(seed)
    atom_order = list(range(n_atoms))
    rng.shuffle(atom_order)
    return Chem.RenumberAtoms(m, atom_order)


def build_tta_dataset(canons: list[str], y: np.ndarray, featurizer, tta_seed: int,
                      normalize_scaler=None) -> data.MoleculeDataset:
    """Build a MoleculeDataset with each canon randomized to a different atom order."""
    pts = []
    for i, smi in enumerate(canons):
        m = Chem.MolFromSmiles(smi)
        if m is None:
            continue
        # Per-molecule seed derived from tta_seed × canon_index to guarantee
        # different orderings per TTA iteration AND per molecule.
        variant = make_random_variant_mol(m, seed=tta_seed * 100003 + i)
        pts.append(data.MoleculeDatapoint(mol=variant, y=y[i]))
    dset = data.MoleculeDataset(pts, featurizer=featurizer)
    if normalize_scaler is not None:
        dset.normalize_targets(normalize_scaler)
    return dset


def predict_with_tta(model, trainer, canons: list[str], scaler,
                     featurizer, n_tta: int, base_tta_seed: int,
                     log: logging.Logger | None = None,
                     ctx: str = "") -> np.ndarray:
    """Predict on canons with N_TTA randomized atom orderings, return mean.
    Includes tta_seed=0 = the canonical (as-parsed) atom ordering, so N_TTA
    total predictions where the first is the standard non-TTA prediction."""
    tta_preds = []

    # First pass: canonical ordering (equivalent to non-TTA baseline)
    y_dummy = np.zeros((len(canons), N_TARGETS), dtype=np.float32)
    canonical_dset = data.MoleculeDataset(
        [data.MoleculeDatapoint(mol=Chem.MolFromSmiles(s), y=y_dummy[i])
         for i, s in enumerate(canons)
         if Chem.MolFromSmiles(s) is not None],
        featurizer=featurizer,
    )
    canonical_loader = data.build_dataloader(canonical_dset, batch_size=BATCH_SIZE,
                                              shuffle=False, num_workers=NUM_WORKERS)
    preds_list = trainer.predict(model, canonical_loader)
    tta_preds.append(torch.cat(preds_list, dim=0).cpu().numpy())

    # Additional TTA iterations with randomized orderings
    for tta_i in range(1, n_tta):
        variant_dset = build_tta_dataset(
            canons, y_dummy, featurizer, tta_seed=base_tta_seed + tta_i,
        )
        variant_loader = data.build_dataloader(variant_dset, batch_size=BATCH_SIZE,
                                                shuffle=False, num_workers=NUM_WORKERS)
        preds_list = trainer.predict(model, variant_loader)
        tta_preds.append(torch.cat(preds_list, dim=0).cpu().numpy())

    stacked = np.stack(tta_preds, axis=0)          # (n_tta, n_valid, n_targets)
    mean_preds = stacked.mean(axis=0)              # (n_valid, n_targets)

    if log is not None:
        # Report cross-TTA variance as a sanity signal
        std_per_row = stacked.std(axis=0).mean(axis=0)   # (n_targets,)
        std_str = "  ".join([f"{t}={std_per_row[i]:.4f}" for i, t in enumerate(TARGETS)])
        log.info(f"[{ctx}] TTA n={n_tta}  cross-view std per target:  {std_str}")
...
def train_single_model_cv(
    ...
    """Train one Chemprop model on a fold. Return (val_preds_tta_averaged, best_epoch, wall_time_min).
    Val prediction uses TTA: N_TTA randomized atom orderings, mean prediction."""
    ...
    wall_train = (time.time() - t0) / 60

    model.eval()

    # TTA-averaged val prediction (N_TTA randomized atom orderings)
    t_tta = time.time()
    val_preds = predict_with_tta(
        model, trainer, val_canons, scaler, featurizer,
        n_tta=N_TTA, base_tta_seed=seed * 7919, log=log, ctx=f"{ctx} TTA-val",
    )
    wall_tta = (time.time() - t_tta) / 60

    wall = wall_train + wall_tta
    best_epoch = trainer.current_epoch - PATIENCE if early_stop.stopped_epoch > 0 else trainer.current_epoch
    log.info(f"[{ctx}] done. train_time={wall_train:.1f}min  tta_time={wall_tta:.1f}min  "
             f"total={wall:.1f}min  final_epoch={trainer.current_epoch}  approx_best_epoch={best_epoch}")
...
def train_single_model_refit(
    ...
    wall_train = (time.time() - t0) / 60
    log.info(f"[{ctx}] refit done. train_time={wall_train:.1f}min")

    # Predict test with TTA (N_TTA randomized atom orderings per test molecule)
    model.eval()
    t_tta = time.time()

    # Filter out unparseable SMILES first (align later)
    valid_indices = []
    valid_canons = []
    for i, smi in enumerate(test_canon_unique):
        if Chem.MolFromSmiles(smi) is not None:
            valid_indices.append(i)
            valid_canons.append(smi)

    tta_preds = predict_with_tta(
        model, trainer, valid_canons, scaler, featurizer,
        n_tta=N_TTA, base_tta_seed=seed * 7919 + 100, log=log, ctx=f"{ctx} TTA-test",
    )
    wall_tta = (time.time() - t_tta) / 60

    aligned = np.full((len(test_canon_unique), N_TARGETS), np.nan, dtype=np.float32)
    for k, orig_i in enumerate(valid_indices):
        aligned[orig_i] = tta_preds[k]

    wall = wall_train + wall_tta
    log.info(f"[{ctx}] test TTA done. tta_time={wall_tta:.1f}min  total={wall:.1f}min")
    return aligned, wall
...
def train_fold_3seed(
    ...
    # Val canons list (used for TTA — need SMILES access for atom shuffling)
    val_canons = [canons[i] for i in val_idxs]
    ...
    for si, seed in enumerate(MODEL_SEEDS):
        ...
        val_preds, best_epoch, wall = train_single_model_cv(
            train_dset, val_dset, val_canons, scaler, seed, featurizer, ctx, log,
        )
...
def main():
    ...
    # ---- GUARD RAIL: compare TTA OOF vs baseline 3-seed (non-TTA) OOF ----
    log.info("=" * 60)
    log.info(f"GUARD RAIL: TTA (N={N_TTA}) OOF vs baseline 3-seed (non-TTA) OOF")
    log.info("=" * 60)
    baseline_3seed_ref = {"eea": 0.9082, "egb": 0.9305, "egc": 0.9070,
                          "ei":  0.7766, "eps": 0.7916, "nc":  0.8681, "tg":  0.9083}
    log.info(f"  {'target':>6s}  {'TTA OOF':>10s}  {'3seed ref':>10s}  {'delta':>10s}  {'verdict':>12s}")
    n_dropped = n_improved = 0
    for t in TARGETS:
        r2 = per_target[t]['oof_r2']
        ref = baseline_3seed_ref[t]
        d = r2 - ref
        if d < -0.005:
            verdict = "DROPPED >0.005"
            n_dropped += 1
        elif d > 0.005:
            verdict = "improved"
            n_improved += 1
            ...
            verdict = "flat (safe)"
        log.info(f"  {t:>6s}  {r2:>10.4f}  {ref:>10.4f}  {d:>+10.4f}  {verdict:>12s}")
    baseline_mean = float(np.mean(list(baseline_3seed_ref.values())))
    log.info(f"  {'MEAN':>6s}  {mean_r2:>10.4f}  {baseline_mean:>10.4f}  "
             f"{mean_r2 - baseline_mean:>+10.4f}")
    log.info("")
    if n_dropped > 0:
        log.warning(f"⚠️  GUARD RAIL: {n_dropped} target(s) dropped OOF R² by more than 0.005 vs baseline. "
                    f"Chemprop may be genuinely SMILES-order-sensitive on those. VERIFY before submitting.")
    if n_improved > 0:
        log.warning(f"⚠️  GUARD RAIL: {n_improved} target(s) improved OOF R² by more than 0.005 — suspicious "
                    f"since Chemprop is theoretically permutation-invariant. Investigate.")
    if n_dropped == 0 and n_improved == 0:
        log.info(f"GUARD RAIL PASSED: all targets within ±0.005 of baseline OOF. Expected behavior for TTA. "
                 f"LB improvement will show up when the blend re-uses this OOF.")

    log.info("=" * 60)
    log.info("FINAL PER-TARGET OOF R²  (3-seed bag + TTA)  vs single-seed Chemprop reference")
    log.info("=" * 60)
    single_ref = {"eea": 0.8883, "egb": 0.9251, "egc": 0.8830,
                  "ei": 0.7804, "eps": 0.7584, "nc": 0.8599, "tg": 0.8934}
    log.info(f"  {'target':>6s}  {'TTA':>10s}  {'single':>10s}  {'delta':>8s}")
    for t in TARGETS:
        r2 = per_target[t]['oof_r2']
        ref = single_ref[t]
        d = r2 - ref
    ...
    log.info(f"  (single-seed Chemprop LB reference: 0.887; 3-seed non-TTA LB reference: 0.892)")
