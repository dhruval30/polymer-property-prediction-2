"""
exp_chain_ext_lgbm_v4rot.py — chain-ext v1 + ONE backbone rigidity scalar.

============================================================================
WHAT'S DIFFERENT FROM CHAIN-EXT LGB v1 (LB 0.894)
============================================================================

**One-line change**: add a single scalar feature — the fraction of backbone
bonds that are rotatable — to the ~14K feature matrix of chain-ext v1.
Everything else (feature stack, LGB hparams, folds, Maxwell prior) is
byte-for-byte identical to v1.

Feature:
  backbone_rot_fraction = n_rotatable_backbone_bonds / (backbone_length - 1)

Where backbone = shortest path between the two `*` wildcards.

============================================================================
WHY A SINGLE SCALAR IS SAFE (unlike v3fixed's 15 domain features)
============================================================================

v3fixed's 15 features got 30-39% gain share on egb/tg — that's 30-40x
over-earning (15/14075 = 0.1% normalized budget). The features encoded
polymer CLASS structure (thiophene 5-pack vs aromatic tg universes), which
overfits by class.

This script's ONE scalar can't over-earn by construction:
  - Normalized budget: 1/14076 = 0.007%
  - A pure ratio (0 to 1) doesn't cluster by polymer class the way
    aromatic-bond-fraction or SMARTS class flags do
  - Vinyl polymers and polyimides can both span 0.1-0.9 on this metric

Kill-switch guard rail (per user's proposal): if the single scalar's gain
share on any target exceeds 2%, that target gets refit WITHOUT the scalar.
The final OOF and submission are always safe by construction.

============================================================================
WHY THIS PHYSICS SCALAR
============================================================================

Backbone rigidity is the CANONICAL Bicerano/van Krevelen predictor of Tg
(rigid backbone → high Tg). Secondary effect on eps (dipole reorientation)
and nc (packing). Not captured by:
  - RDKit's `NumRotatableBonds` (global count, includes sidechains)
  - `backbone_atom_count` (measures length, not rigidity)
  - Chain extension (repeats monomer patterns, doesn't create new rot ratio)

So it's genuinely orthogonal to the existing ~14K features.

============================================================================
EXPECTED
============================================================================

Solo LB: +0.001 to +0.003 over chain-ext v1's 0.894 → **0.895-0.897**.
Bounded upside because a single scalar can't unlock huge signal.
Bounded downside because the guard rail prevents overfit-drag scenarios.

If it works (LB ≥ 0.895), we've validated the "one-scalar-at-a-time"
pattern and can consider adding a second scalar (e.g., F-density per
backbone) with same guard rail.

============================================================================
ORIGINAL v1 DESCRIPTION FOLLOWS
============================================================================

Extends each polymer `*A*` SMILES to a trimer `*AAA*` (3 units chained
head-to-tail), computes fingerprints + descriptors on BOTH the monomer AND
the trimer, and concatenates as separate feature families in a LightGBM
...
============================================================================
OUTPUTS  (under results/exp_chain_ext_lgbm_v4rot/)
============================================================================

  run.log            — training logs + guard rail decisions + Maxwell fit stats
  oof.csv            — OOF predictions after Maxwell blend
  submission.csv     — Kaggle format id, target
  cv_summary.json    — per-target R², Maxwell params, rot_frac gain shares, config
...
============================================================================
  ...
  poly2-venv/bin/python experiments/exp_chain_ext_lgbm_v4rot.py
...
EXP_NAME = "exp_chain_ext_lgbm_v4rot"
EXP_DIR = REPO / "results" / EXP_NAME
FEATURE_CACHE_PATH = EXP_DIR / "feature_cache.pkl"

# Guard rail: if backbone_rotfrac gain share > this on any target, refit
# that target WITHOUT the scalar. Prevents v3fixed-style class-encoding overfit.
GAIN_SHARE_CEILING = 0.02
...
# Number of monomer units in the chain extension
CHAIN_N_UNITS = 3
...
# ============================================================================
# NEW: SINGLE SCALAR — backbone rotatable-bond fraction
# ============================================================================
#
# Rationale in header docstring. This is the ONLY new feature added to v1's
# ~14K feature stack. Single scalar; guard-railed by GAIN_SHARE_CEILING.

_ROT_BOND_PATT = Chem.MolFromSmarts("[!$(*#*)&!D1]-&!@[!$(*#*)&!D1]")


def compute_backbone_rot_fraction(smi: str) -> float:
    """Rotatable backbone bonds / (backbone length - 1). NaN on failure or
    when polymer doesn't have exactly 2 wildcards."""
    m = Chem.MolFromSmiles(smi)
    if m is None:
        return float("nan")
    stars = [a.GetIdx() for a in m.GetAtoms() if a.GetSymbol() == "*"]
    ...
    if len(path) < 2:
        return float("nan")
    try:
        rot_bond_pairs = {frozenset(b) for b in m.GetSubstructMatches(_ROT_BOND_PATT)}
    except Exception:
        return float("nan")
    backbone_bond_pairs = [frozenset((path[i], path[i + 1])) for i in range(len(path) - 1)]
    n_rot_backbone = sum(1 for bp in backbone_bond_pairs if bp in rot_bond_pairs)
    denom = max(1, len(path) - 1)
    return n_rot_backbone / denom
...
def build_feature_bundle(canon_smiles: list[str], log: logging.Logger) -> dict:
    ...
    t0 = time.time()
    X = np.array(
        [compute_backbone_rot_fraction(s) for s in tqdm(smis_mono, desc="backbone rot frac", ncols=100)],
        dtype=np.float32,
    ).reshape(-1, 1)
    _add("backbone_rotfrac", X)
    n_nan = int(np.isnan(X).sum())
    log.info(f"  backbone_rotfrac: {X.shape}  time={time.time()-t0:.1f}s  "
             f"n_nan={n_nan} ({100*n_nan/len(smis_mono):.1f}%)  "
             f"mean={float(np.nanmean(X)):.3f}  std={float(np.nanstd(X)):.3f}")
...
def slice_smiles_features(bundle: dict, canon_series: pd.Series,
                          exclude_family: str | None = None) -> np.ndarray:
    """If exclude_family is set, drop that family's columns from the returned X."""
    idx = canon_series.map(bundle["smiles_index"]).values
    X = bundle["X"][idx]
    if exclude_family is not None and exclude_family in bundle["families_slice"]:
        sl = bundle["families_slice"][exclude_family]
...
def train_one_target(
    ...
    X_tr_smi = slice_smiles_features(bundle, g_tr["canon"], exclude_family=exclude_family)
    X_te_smi = slice_smiles_features(bundle, g_te["canon"], exclude_family=exclude_family)
    if exclude_family is not None:
        log.info(f"[{target}] EXCLUDING family '{exclude_family}' (guard-rail retrain)")
    ...
    # If exclude_family is set, that family's columns weren't in X — walk families
    # in order and track cursor through the REDUCED column space.
...
def main():
    ...
    # ==== GUARD RAIL: check backbone_rotfrac gain share per target ====
    log.info("=" * 60)
    log.info(f"GUARD RAIL: backbone_rotfrac gain share (ceiling = {100*GAIN_SHARE_CEILING:.1f}%)")
    log.info("=" * 60)
    tripped: list[str] = []
    for tgt in TARGETS:
        br_gain = results[tgt]["family_gains"].get("backbone_rotfrac", 0)
        total_gain = results[tgt]["total_gain"]
        share = br_gain / max(1, total_gain)
        marker = "TRIPPED" if share > GAIN_SHARE_CEILING else "safe"
        log.info(f"  [{tgt:>4s}] backbone_rotfrac gain = {br_gain}  share = {100*share:.3f}%  ({marker})")
        if share > GAIN_SHARE_CEILING:
            tripped.append(tgt)

    if tripped:
        log.warning(f"GUARD RAIL TRIPPED on {len(tripped)} targets: {tripped}")
        log.warning(f"Refitting those targets WITHOUT backbone_rotfrac feature...")
        for tgt in tripped:
            results[tgt] = train_one_target(
                tgt, tr, te, bundle, aux_lookup, log, exclude_family="backbone_rotfrac",
            )
        log.info(f"Refit complete for {len(tripped)} targets. Submission will be safe by construction.")
    else:
        log.info("ALL TARGETS SAFE — backbone_rotfrac stayed under the gain-share ceiling on every target.")

    baseline_mean_r2 = float(np.mean([results[t]["oof_r2"] for t in TARGETS]))
    log.info("=" * 60)
    log.info("LGB-only per-target OOF R²  (before Maxwell, after guard rail)")
    log.info("=" * 60)
    for t in TARGETS:
    ...
    # Final comparison vs chain-ext v1 reference (LB 0.894)
    log.info("=" * 60)
    log.info("FINAL PER-TARGET OOF R²  (v4rot)  vs chain-ext v1 reference")
    log.info("=" * 60)
    v1_ref = {"eea": 0.8734, "egb": 0.9087, "egc": 0.9023,
              "ei":  0.8041, "eps": 0.8218, "nc":  0.8471, "tg":  0.9063}
    log.info(f"  {'target':>6s}  {'v4rot':>10s}  {'v1 ref':>10s}  {'delta':>8s}  {'rot excl':>10s}")
    for t in TARGETS:
        r2 = per_target[t]["oof_r2"]
        ref = v1_ref[t]
        d = r2 - ref
        excl = "YES" if per_target[t]["excluded_family"] == "backbone_rotfrac" else "no"
        log.info(f"  {t:>6s}  {r2:>10.4f}  {ref:>10.4f}  {d:>+8.4f}  {excl:>10s}")
    v1_mean = float(np.mean(list(v1_ref.values())))
    log.info(f"  {'MEAN':>6s}  {mean_r2:>10.4f}  {v1_mean:>10.4f}  {mean_r2 - v1_mean:>+8.4f}")
    log.info(f"  (chain-ext v1 LB reference: 0.894 — 'rot excl' = YES means guard rail tripped and scalar was excluded for that target)")
