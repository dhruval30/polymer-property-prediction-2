"""
exp_chain_ext_lgbm.py — LGB pipeline with polymer chain extension features + Maxwell prior.
...
def main():
    ...
    # Final comparison vs maxwell_prior reference (monomer-only LGB with Maxwell, LB 0.860)
    log.info("=" * 60)
    log.info("FINAL PER-TARGET OOF R²  (chain-ext + Maxwell)  vs monomer-only LGB reference")
    log.info("=" * 60)
    monomer_ref = {"eea": 0.8543, "egb": 0.9050, "egc": 0.8966,
                   "ei": 0.7944, "eps": 0.8186, "nc": 0.8603, "tg": 0.9026}
    log.info(f"  {'target':>6s}  {'chain-ext':>10s}  {'mono ref':>10s}  {'delta':>8s}  {'tri gain':>10s}")
    for t in TARGETS:
        r2 = per_target[t]["oof_r2"]
        ref = monomer_ref[t]
        d = r2 - ref
        log.info(f"  {t:>6s}  {r2:>10.4f}  {ref:>10.4f}  {d:>+8.4f}  {per_target[t]['tri_gain_share']:>9.1f}%")
    mono_mean = float(np.mean(list(monomer_ref.values())))
    log.info(f"  {'MEAN':>6s}  {mean_r2:>10.4f}  {mono_mean:>10.4f}  {mean_r2 - mono_mean:>+8.4f}")
    log.info(f"  (monomer-only LGB LB reference: 0.860)")
