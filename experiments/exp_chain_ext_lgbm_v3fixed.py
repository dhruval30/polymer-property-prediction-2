"""
exp_chain_ext_lgbm_v3fixed.py — v1 + safe additive upgrades (no IterImputer).

============================================================================
WHY THIS EXISTS
============================================================================

v3 tried to add IterativeImputer for aux features but had a fold-alignment
bug that leaked val fold targets → OOF 0.895 but LB 0.601 (catastrophic).

v3-fixed keeps the 3 SAFE additive changes from v3 and rips out IterImputer
entirely. Reverts to v1's proven single-pass aux (LGB handles NaN natively).

Additions to v1:
  (1) Nc-fix — drop trimer features for nc target only (v1 log showed
      nc regressed -0.013 with trimer; mono-only should recover)
  (2) Bandgap post-processor for egb + ei only (v2 log validated these
      as safe blends: egb Δ+0.006, ei Δ+0.004 vs LGB alone)
  (3) 15 domain-knowledge features (research doc §5.2 — 14th-place NeurIPS
      2025 recipe: F counts, backbone rigidity, Kier-Hall proxies, SMARTS)

Nothing else changes. Same feature stack, same LGB hparams, same 5-fold
GroupKFold with seed=42, same aux, same Maxwell prior.

Expected LB: 0.895-0.898 (recovers v1's 0.894, adds +0.001 to +0.004 from
the additive stack). No risk of another IterImputer-style disaster.
...
EXP_NAME = "exp_chain_ext_lgbm_v3fixed"
EXP_DIR = REPO / "results" / EXP_NAME
FEATURE_CACHE_PATH = EXP_DIR / "feature_cache.pkl"

# v3-fixed: bandgap post-processor config (matches v3 behavior, egb + ei only)
...
# 15 domain-knowledge features (research doc §5.2, 14th-place NeurIPS 2025 recipe)
...
def compute_domain_features(smi: str) -> np.ndarray:
    """15 hand-engineered features on MONOMER SMILES.  Zero on parse failure."""
    ...
    i_count = sum(1 for a in m_capped.GetAtoms() if a.GetSymbol() == "I")
    ...
    out[4] = f_count / heavy
    out[5] = (f_count + cl_count + br_count + i_count) / heavy
...
def build_feature_bundle(canon_smiles: list[str], log: logging.Logger) -> dict:
    ...
    # -------- NEW: 15 DOMAIN features on MONOMER (research doc §5.2) --------
...
def slice_smiles_features(bundle: dict, canon_series: pd.Series, drop_trimer: bool = False) -> np.ndarray:
    """If drop_trimer=True, only keep mono+domain feature columns (used for nc-fix)."""
...
def train_one_target(
    ...
    drop_trimer = (target == "nc")   # NC-FIX: mono-only features for nc
    if drop_trimer:
    ...
    if drop_trimer:
        # nc uses mono-only slice — walk through mono families in order to get proper slices
        cursor = 0
        for fam, sl in bundle["families_slice"].items():
            if fam.endswith("_mono"):
    ...
    # Split into mono / tri / domain / aux totals
...
def bandgap_post_process(
    ...
    """Physics blends for egb (from egc) and ei (from egc + eea). Grid-searched
    weights on OOF; only applied when Δ > BANDGAP_MIN_DELTA. Same weight to test."""
    log.info("=" * 60)
    ...
    for target in BANDGAP_TARGETS:
        ...
        log.info(f"  [{target}] n_phys_valid={n_phys_valid}/{len(oof_df)}   "
                 f"LGB R²={r2_lgb:.4f}   best w_lgb={best_w:.3f}   "
                 f"blend R²={best_r2:.4f}   Δ={delta:+.4f}")
        ...
        if delta > BANDGAP_MIN_DELTA:
            ...
            summary[target] = {"applied": True, "w_lgb": best_w, "r2_before": r2_lgb,
                               "r2_after": best_r2, "delta": delta}
        else:
            log.info(f"  [{target}] SKIP bandgap blend (Δ={delta:+.4f} ≤ {BANDGAP_MIN_DELTA})")
            summary[target] = {"applied": False, "w_lgb": None, "r2_before": r2_lgb,
                               "r2_after": r2_lgb, "delta": delta}
    return summary
...
def main():
    ...
    summary = {
        ...
        "mean_r2_post_bandgap":         post_bandgap_mean,
    ...
    log.info("=" * 60)
    log.info("FINAL PER-TARGET OOF R²  (v3fixed)  vs chain-ext v1 (3-mer) reference")
    log.info("=" * 60)
    v1_ref = {"eea": 0.8734, "egb": 0.9087, "egc": 0.9023,
              "ei":  0.8041, "eps": 0.8218, "nc":  0.8471, "tg":  0.9063}
    log.info(f"  {'target':>6s}  {'v3fixed':>10s}  {'v1 ref':>10s}  {'delta':>8s}  {'tri gain':>10s}  {'domain':>8s}")
    for t in TARGETS:
        r2 = per_target[t]["oof_r2"]
        ref = v1_ref[t]
        d = r2 - ref
        log.info(f"  {t:>6s}  {r2:>10.4f}  {ref:>10.4f}  {d:>+8.4f}  "
                 f"{per_target[t]['tri_gain_share']:>9.1f}%  {per_target[t]['domain_gain_share']:>7.1f}%")
